...
{
    std::string line;
    std::vector<PointType> start_choices;
    PointType end;
    ...
    for (int y = 0; std::getline(file, line); y++)
    {
        ...
        if (x != std::string::npos)
            start_choices.push_back(std::make_pair(x, y));
    ...
    for (auto &start : start_choices)
    {
        ...
        int steps_at_end = steps[std::get<Y>(end)][std::get<X>(end)];
        if (steps_at_end && steps_at_end < fewest_steps)
        {
            fewest_steps = steps_at_end;
#ifdef DPRINT_ON
            DPRINT2(sx, sy);
            std::cout << steps_at_end << std::endl;
