...
std::optional<uint64_t> Answer(std::ifstream &file)
{
    ...
    struct Solution
    {
        ...
        Solution(const Position &start,
                 const Position &end,
                 int minutes_at_start)
            : current(start), goal(end), minutes(minutes_at_start) {}
    ...
    auto trip = [&](const Position &start,
                    const Position &end,
                    int minutes_at_start) -> int
    {
        solution_queue.push(Solution(start, end, minutes_at_start));
        int fewest_number_of_minutes = INT_MAX;

        while (true)
        {
            if (visited_at_this_minute.size() > minutes_at_start)
            {
                visited_at_this_minute.pop_back();
                continue;
            }
            break;
        }
        ...
        while (true)
        {
            if (visited_at_this_minute.size() < drawings.size())
            {
        ...
        while (true)
        {
            ...
            bool reached_goal = current == solution.goal;
            ...
            if (fewest)
            {
                ...
                std::cout << (reached_goal ? (fewest ? "fewest goal "
                                                     : "goal ")
                                           : (abort ? "abort " : ""))
                          << "[queue size "
                          << solution_queue.size() + 1 << "] "
                          << std::endl;
...
#ifdef DPRINT_ON
        std::cout << fewest_number_of_minutes - minutes_at_start
                  << " minutes from ";
        DPRINT(start);
        std::cout << " to ";
        DPRINT(end);
    ...
    return trip(non_wall_top, non_wall_bottom,
                trip(non_wall_bottom, non_wall_top,
                     trip(non_wall_top, non_wall_bottom, 0)));
}
