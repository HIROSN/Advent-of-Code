...
int all_version_numbers(const Packet &packet);
...
uint64_t Answer(std::ifstream &file)
{
    ...
    {
        return all_version_numbers(packet);
    }
...
int all_version_numbers(const Packet &packet)
{
    int version_numbers = packet.version;

    for (const auto &sub_packet : packet.sub_packets)
    {
        version_numbers += all_version_numbers(sub_packet);
    }
