...
std::string one_higher(std::string line);
...
uint64_t Answer(std::ifstream &file)
{
    ...
    for (int y = 0; y < size_y * 4; y += size_y)
    {
        for (int i = 0; i < size_y; i++)
        {
            cavern.push_back(one_higher(cavern[y + i]));
        }
    }

    for (auto &line : cavern)
    {
        for (int x = 0; x < size_x * 4; x += size_x)
        {
            line += one_higher(line.substr(x, size_x));
        }
    }
