...
std::optional<uint64_t> Answer(std::ifstream &file)
{
    ...
    struct Hand
    {
        ...
        Hand(const std::string &str, int i) : bid(i)
        {
            ...
            if (type.stack_size == 1)
            {
                type.highest_stack = str.size();
            }
            else
            {
                for (auto it : stack)
                {
                    if (it.first != joker)
                    {
                        type.highest_stack = std::max(
                            {type.highest_stack, it.second});
                    }
                }

                if (stack.find(joker) != stack.end())
                {
                    type.stack_size--;
                    type.highest_stack += stack[joker];
                }
            }
        }
