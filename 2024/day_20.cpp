...
std::optional<uint64_t> Answer(std::ifstream &file)
{
    ...
    {
        const Number &x = it.first.first;
        const Number &y = it.first.second;

        for (auto o : aoc::FourNeighbors().offsets)
        {
            if (!is_wall(x + o.x, y + o.y))
                continue;

            Cheat cheat{{x, y}, {x + o.x * 2, y + o.y * 2}};

            if (is_wall(cheat.second.x, cheat.second.y))
                continue;

            auto use_cheat = no_cheat_fewest(x, y) + 2;
            auto no_cheat = no_cheat_fewest(cheat.second.x, cheat.second.y);
